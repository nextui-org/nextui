...
const expectCollapseToHaveARIAExpanded = (
  ...
  expect(getCollapseElement(wrapper, className).props()["aria-expanded"]).toBe(value);
};
...
describe("Collapse Group", () => {
  ...
  it("should be no errors when children are missing", () => {
    const wrapper = mount(<Collapse.Group />);
  ...
  it("should trigger event when collapse changed", () => {
    ...
    const callback = jest.fn().mockImplementation((idx: number | undefined, val: boolean) => {
      index = idx || 0;
      value = val;
    });
