...
describe("Loading", () => {
  ...
  it("should render with default aria-label", () => {
    ...
    expect(wrapper.find(".nextui-loading").at(0).prop("aria-label")).toEqual("Loading");
  });

  it("should render with default aria-label for spinner", () => {
    const wrapper = mount(<Loading type="spinner" />);

    expect(wrapper.find(".nextui-spinner-container").at(0).prop("aria-label")).toEqual("Loading");
  });

  it("should work with children", () => {
    const wrapper = mount(<Loading>Loading</Loading>);

    expect(wrapper.find(".nextui-loading-label").at(0).text()).toContain("Loading");
  });
