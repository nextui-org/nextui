...
describe("Snippet", () => {
  ...
  it("should work with different styles", () => {
    const wrapper = mount(
      <div>
        ...
        <Snippet filled text={command} />
        <Snippet color="secondary" text={command} />
        <Snippet filled color="success" text={command} />
      </div>,
    );
  ...
  it("should disable copy function", () => {
    const wrapper = mount(<Snippet copy="prevent" text={command} />);

    expect(wrapper.find(".nextui-snippet-copy-button").length).toBe(0);
  });

  it("should work with custom symbol", () => {
    const wrapper = mount(<Snippet symbol=">" text={command} />);

    expect(wrapper.html()).toMatchSnapshot();
    expect(() => wrapper.unmount()).not.toThrow();

    const emptySymbolWrapper = mount(<Snippet symbol=" " text={command} />);

    expect(emptySymbolWrapper.html()).toMatchSnapshot();

    const emptySymbolWrapper2 = mount(<Snippet symbol="" text={command} />);
  ...
  it("should work with custom toast", () => {
    document.execCommand = jest.fn();
    const wrapper = mount(<Snippet text={command} tooltipCopiedText="Code copied!" />);
