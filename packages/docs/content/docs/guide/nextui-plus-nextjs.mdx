...
NextUI is totally compatible with <Link target="_blank" color rel="nofollow" href="https://nextjs.org">Next.js</Link> you just need to customize the <Link target="_blank" color rel="nofollow" href="#">app.jsx</Link> entry file to load the provider.

Start your Next.js project with <Link target="\_blank" color rel="nofollow" href="#"> this example</Link>.

### Server Render

All Components of `@nextui/react` are compatible with **Server Render**. In fact, the page you see now is rendered by the server.
...
Here's an example of what your `_document.js` file should look like: <Link target="_blank" href="https://github.com/jrgarciadev/nextui/blob/main/packages/docs/src/pages/_document.tsx" rel="nofollow" color>\_document.jsx</Link>.
