...
This will enable the dark mode for the whole application. But we know that applications
not always have only one theme, you usually need a way to switch between themes. To do so
we recommend to use a `theme` switch library or create your own. 

## Using next-themes

If you're using [Next.js](/docs/frameworks/nextjs) the best option at the moment is to use [next-themes](https://github.com/pacocoursey/next-themes)
library. It comes with features that improves the experience of switching between themes. 

> See the [next-themes](https://github.com/pacocoursey/next-themes) documentation for more details.
