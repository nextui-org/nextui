...
In your Next.js project, run one of the following command to install HeroUI:
...
> **Important 🚨**: Note that you need to import the component from the individual package, not the from `@heroui/react`. 
...
Go to `root` file and wrap the Component with the `HeroUIProvider`:
...
<Blockquote
 color="default"
>
  Please visit the <Link href="https://github.com/frontio-ai/heroui/releases/tag/v2.0.0" showAnchorIcon isExternal>Release Notes</Link> for more information about the new features and breaking changes.
</Blockquote>
