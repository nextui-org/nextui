...
<CodeDemo title="Width Tooltip" files={sliderContent.tooltip} />
...
It is also possible to format the value using the `getOutputValue` prop.

<CodeDemo title="Value Formatting (Get output value)" files={sliderContent.getOutputValue} />

### Hiding the Output

The Slider output/value is shown by default, but can be hidden by passing the `hideOutput` prop.

<CodeDemo title="Hiding the Output" files={sliderContent.hideOutput} />
...
The Slider component provides a `renderOutput` prop that allows you to customize the output element.

<CodeDemo title="Custom Output" files={sliderContent.renderOutput} />
...
- **labelWrapper**: The container for the Slider's label and output. It aligns these elements and ensures a consistent layout.
- **label**: A dedicated slot to display the Slider's label.
- **output**: Displays the current value of the Slider. Located within the `labelWrapper`.
...
| classNames       | `Record<"base"｜"labelWrapper"｜"label"｜ "output" ｜ "step" ｜ "mark" ｜ "trackWrapper" ｜ "track"｜ "output" ｜ "filler" ｜ "thumb" ｜ "startContent" ｜ "endContent", string>` | Allows to set custom class names for the Slider slots.                                                                                                        | -            |
