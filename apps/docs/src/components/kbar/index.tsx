...
import {KBarPortal, KBarPositioner, KBarAnimator, VisualState, useKBar} from "kbar";
...
const KBar: React.FC<unknown> = () => {
  ...
  return (
    <KBarPortal>
      <Backdrop blur className="backdrop" maxWidth={styles.container.maxWidth} visible={visible}>
        <KBarPositioner>
          <KBarAnimator style={styles.container}>
            <KBarSearch placeholder="What do you need?" />
            <KBarResults
              style={styles.result}
              onRender={(action: Action, handlers: ResultHandlers, state: ResultState) => (
